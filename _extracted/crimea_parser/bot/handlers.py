...
def cmd_help(token: str, chat_id: int, user_id: int, args: str) -> None:
    text = (
        ...
        "  /run_source &lt;ключ&gt; — точечный запуск\n     (osm/wikidata/wikipedia/gosreestr/vk/yandex/search/2gis/avito/sutochno/ostrovok/crawler)\n"
    )
...
def cmd_run_source(token: str, chat_id: int, user_id: int, args: str) -> None:
    ...
    allowed = {"osm", "wikidata", "wikipedia", "gosreestr", "vk", "yandex",
               "search", "2gis", "avito", "sutochno", "ostrovok", "crawler"}
